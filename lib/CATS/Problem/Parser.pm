...
sub validate
{
    ...
    $check_order->($self->{problem}{tests}, 'test');
    my @t = values %{$self->{problem}{tests}};
    ...
    $check_order->($self->{problem}{samples}, 'sample');

    $self->{problem}{run_method} ||= $cats::rm_default;

    # insert_problem_content
    $self->{problem}{has_checker} or $self->error('No checker specified');
}
...
sub start_tag_Problem
{
    ...
    for ($self->{problem}{description}{memory_limit}) {
        last if defined $_;
        $_ = 200;
        $self->warning("Problem.mlimit not specified. default: $_");
    }

    if ($self->{problem}{description}{std_checker}) {
        $self->warning("Deprecated attribute 'stdChecker', use Import instead");
        $self->checker_added;
    }

    my $ot = $self->{problem}{old_title};
    $self->error(sprintf
        "Unexpected problem rename from: $ot to: $self->{problem}{description}{title}",
    ) if $ot && $self->{problem}{description}{title} ne $ot;
}
...
sub start_tag_Testset
{
    (my CATS::Problem::Parser $self, my $atts) = @_;
    my $n = $atts->{name};
    $self->{problem}{testsets}->{$n} and $self->error("Duplicate testset '$n'");
    $self->parse_test_rank($atts->{tests});
    $self->{problem}{testsets}->{$n} = {
        id => new_id,
        map { $_ => $atts->{$_} } qw(name tests points comment hideDetails)
    };
    $self->{problem}{testsets}->{$n}->{hideDetails} ||= 0;
